...
\begin{enumerate}[start=1,label=Q\arabic*,left=0pt]
    ...
    \item \textbf{Given the initial probability distribution} $p_0 = [0.1, 0.6, 0.3]$ \textbf{for states} $S_1, S_2,$ \textbf{and} $S_3$ \textbf{respectively, compute the stationary distribution for the given Markov model. You may use a program to do the computations. Report only the final stationary distribution} ${\pi}$.
    ...
    \item \textbf{Given the starting probability distribution $p0 = [0.1, 0.6, 0.3]$, what is the probability of ending up in $S2$ after exactly two transitions? (Show your calculations.)}
