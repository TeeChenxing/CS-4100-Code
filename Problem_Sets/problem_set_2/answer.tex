...
\begin{enumerate}[start=8,label=Q\arabic*,left=0pt]
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \definecolor{customblue}{rgb}{0, 0, 1} % Example RGB for blue
            \definecolor{custompurple}{rgb}{0.5, 0.0, 0.5} % Example RGB for purple
        
            % Define node styles
            \tikzstyle{state} = [circle, draw, fill=customblue!20, minimum size=2em]
            \tikzstyle{action} = [circle, draw, fill=custompurple!20, minimum size=2em]
            \tikzstyle{arrow} = [->, thick]
        
            % Nodes
            \node[state] (S1) {S1};
            \node[action, right of=S1, node distance=3cm] (A2) {A2};
            \node[state, right of=A2, node distance=3cm] (S3) {S3};
            \node[action, right of=S3, node distance=3cm] (A4) {A4};
            \node[state, right of=A4, node distance=3cm] (S4) {S4};
